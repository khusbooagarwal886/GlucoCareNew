...
import { buildMedicationAdherenceSummary } from "@/lib/medication-adherence";
import MedicationAdherenceSummaryCard from "@/components/medication/MedicationAdherenceSummaryCard";
...
const FamilyDashboard = ({ onLogout }: FamilyDashboardProps) => {
  ...
  useEffect(() => {
    ...
    async function loadMedicationAdherence() {
      ...
      try {
        ...
        await autoMarkMissedMedicationDoses(linkedPatient.uid, 7);
        const [schedulesByDate, logs] = await Promise.all([
          Promise.all(recentDates.map((date) => listMedicationScheduleForDate(linkedPatient.uid, date))),
          listMedicationAdherenceLogs(linkedPatient.uid, 7),
        ]);
        if (!cancelled) {
  ...
  const caregiverInsights = useMemo<HealthInsight[]>(() => {
    ...
    if (medicationAdherenceSummary.totalExpected > 0) {
      items.push({
        id: "family-medication-adherence",
        patientId: pid,
        type: "trend",
        category: "medication",
        title: "Medication Adherence Watch",
        description: `${patientName || "Patient"} completed ${medicationAdherenceSummary.adherenceRate}% of scheduled doses over the last 7 days.`,
        timestamp: new Date().toISOString(),
        actionItems:
          medicationAdherenceSummary.skipped > 0
            ? ["Check in on skipped doses and confirm if reminders are being followed."]
            : ["Medication routine looks consistent this week."],
      });
    }

    return items;
  }, [diabetesRiskSummary, linkedPatient?.patientId, medicationAdherenceSummary.adherenceRate, medicationAdherenceSummary.skipped, medicationAdherenceSummary.totalExpected, patientName, stats.average, stats.inRangePercentage, stats.readings, unreadAlerts]);
  ...
  return (
    <div className="min-h-screen bg-gradient-hero">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.47 }} className="mb-8">
          <MedicationAdherenceSummaryCard
            title="Medication Adherence (7 Days)"
            summary={medicationAdherenceSummary}
            emptyMessage="No scheduled medication doses are available for this patient yet."
          />
        </motion.div>
