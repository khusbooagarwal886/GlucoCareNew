...
const DoctorDashboard = ({ onLogout }: DoctorDashboardProps) => {
  ...
  useEffect(() => {
    ...
    async function loadSelectedPatientData() {
      ...
      try {
        ...
        await autoMarkMissedMedicationDoses(selectedPatient.uid, 7);
        const [metrics, notes, plans, schedulesByDate, adherenceLogs] = await Promise.all([
          ...
          Promise.all(recentDates.map((date) => listMedicationScheduleForDate(selectedPatient.uid, date))),
          listMedicationAdherenceLogs(selectedPatient.uid, 7),
        ]);
        if (!cancelled) {
          ...
          setMedicationAdherenceLogs(adherenceLogs);
        }
      } catch (error) {
  ...
  const refreshCarePlanData = async () => {
    ...
    const recentDates = buildRecentDates(7);
    await autoMarkMissedMedicationDoses(selectedPatient.uid, 7);
    const [schedulesByDate, adherenceLogs] = await Promise.all([
      Promise.all(recentDates.map((date) => listMedicationScheduleForDate(selectedPatient.uid, date))),
      listMedicationAdherenceLogs(selectedPatient.uid, 7),
    ]);
    setMedicationScheduleEntries(schedulesByDate.flat());
