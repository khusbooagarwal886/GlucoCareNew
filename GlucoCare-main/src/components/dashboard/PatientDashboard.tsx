...
import {
  ...
  upsertActivityLog,
  upsertMedicationAdherenceLog,
  type ActivityLogDoc,
  type MedicationAdherenceLogDoc,
...
import LogMealDialog from '@/components/meal/LogMealDialog';
import {
  buildMedicationAdherenceSummary,
...
const PatientDashboard = ({ onLogout }: PatientDashboardProps) => {
  ...
  const [mealLogs, setMealLogs] = useState<MealLogDoc[]>([]);
  ...
  const [medicationAdherenceLogs, setMedicationAdherenceLogs] = useState<MedicationAdherenceLogDoc[]>([]);
  const [medicationAdherenceSavingKey, setMedicationAdherenceSavingKey] = useState<string | null>(null);
  ...
  const todayMedicationDate = useMemo(() => formatDateYmdLocal(new Date()), []);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [readings, activityItems, mealItems] = await Promise.all([
        listDailyHealthMetrics(user.uid),
        listActivityLogs(user.uid),
        listMealLogs(user.uid),
      ]);
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    async function loadMedicationAdherence() {
      if (!user) {
        ...
        setMedicationAdherenceLogs([]);
        return;
      }

      try {
        const recentDates = buildRecentDates(7);
        await autoMarkMissedMedicationDoses(user.uid, 7);
        const [schedulesByDate, logs] = await Promise.all([
          Promise.all(recentDates.map((date) => listMedicationScheduleForDate(user.uid, date))),
          listMedicationAdherenceLogs(user.uid, 7),
        ]);

        if (cancelled) return;
        setMedicationScheduleEntries(schedulesByDate.flat());
        setMedicationAdherenceLogs(logs);
      } catch {
        if (!cancelled) {
          setMedicationScheduleEntries([]);
          setMedicationAdherenceLogs([]);
  ...
  const todayMedicationSchedule = useMemo(
    () => medicationScheduleEntries.filter((entry) => entry.scheduleDate === todayMedicationDate),
    [medicationScheduleEntries, todayMedicationDate]
  );

  const medicationAdherenceSummary = useMemo(
    () => buildMedicationAdherenceSummary(medicationScheduleEntries, medicationAdherenceLogs),
    [medicationAdherenceLogs, medicationScheduleEntries]
  );

  const medicationAdherenceByKey = useMemo(
    () =>
      new Map(
        medicationAdherenceLogs.map((log) => [`${log.scheduleDate}_${log.slot}`, log] as const)
      ),
  ...
  const panelAlerts = useMemo<Alert[]>(() => {
    ...
    return notifications.map((n) => ({
      id: n.id,
      patientId,
      type: n.type === "medication-adherence" ? "medication-reminder" : "appointment",
      severity:
        n.type === "medication-adherence"
          ? "warning"
          : n.status === "rejected"
            ? "warning"
            : "info",
  ...
  const derivedInsights = useMemo<HealthInsight[]>(() => {
    ...
    if (medicationAdherenceSummary.totalExpected > 0) {
      items.push({
        ...
        description: `You completed ${medicationAdherenceSummary.adherenceRate}% of scheduled doses over the last 7 days.`,
        timestamp: new Date().toISOString(),
        actionItems:
          medicationAdherenceSummary.skipped > 0
            ? ["Review missed doses and update each reminder after taking medicine."]
            : ["Great consistency. Keep marking each dose after you take it."],
      });
    }

    return items;
  }, [diabetesRiskSummary, latestDaily?.bloodPressureDiastolic, latestDaily?.bloodPressureSystolic, latestDaily?.heartRate, medicationAdherenceSummary.adherenceRate, medicationAdherenceSummary.skipped, medicationAdherenceSummary.totalExpected, patientProfile?.patientId, stats.average, stats.inRangePercentage, stats.readings, user?.uid]);
  ...
  const handleMedicationStatusUpdate = async (
    entry: MedicationScheduleEntry,
    status: MedicationAdherenceStatus
  ) => {
    if (!user) return;
    const saveKey = `${entry.scheduleDate}_${entry.slot}`;
    setMedicationAdherenceSavingKey(saveKey);
    try {
      await upsertMedicationAdherenceLog(user.uid, {
      ...
      const logs = await listMedicationAdherenceLogs(user.uid, 7);
      setMedicationAdherenceLogs(logs);
      toast.success(`Marked ${entry.slotLabel.toLowerCase()} as ${status}.`);
    } catch {
      toast.error("Failed to update medication status.");
    } finally {
      setMedicationAdherenceSavingKey(null);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        >
          ...
          <MetricCard
            title="Med Adherence"
            value={medicationAdherenceSummary.totalExpected > 0 ? medicationAdherenceSummary.adherenceRate : "--"}
            unit={medicationAdherenceSummary.totalExpected > 0 ? "%" : "no plan"}
            icon={Pill}
            status={
              medicationAdherenceSummary.totalExpected === 0
                ? "normal"
                : medicationAdherenceSummary.adherenceRate >= 80
                ? "success"
                : medicationAdherenceSummary.adherenceRate >= 50
                  ? "warning"
                  : "danger"
            }
          />
          ...
          <Card variant="glass">
            ...
            <CardContent className="space-y-4">
              ...
              {!mealLogsLoading && mealLogs.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  No meals logged yet. Add meals from search or upload a meal photo to start tracking.
                </p>
              )}
              {mealLogs.slice(0, 5).map((meal) => (
                <div key={meal.id} className="rounded-lg border border-border/60 p-4 space-y-3">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div>
                      <p className="font-semibold capitalize">{meal.mealType}</p>
                      <p className="text-xs text-muted-foreground">
                        {meal.loggedAt.toLocaleString()}
                      </p>
                    </div>
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    {meal.foods.map((food, idx) => (
                      <div key={`${meal.id}-food-${idx}`} className="rounded-md bg-muted/30 p-3">
        ...
        >
          <Card variant="glass">
            ...
            <CardContent className="space-y-3">
              {todayMedicationSchedule.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  Your doctor has not added a timed prescription yet.
                </p>
              )}
              {todayMedicationSchedule.map((entry) => {
                const key = `${entry.scheduleDate}_${entry.slot}`;
                const currentLog = medicationAdherenceByKey.get(key);
                return (
                  <div key={key} className="rounded-lg border border-border/60 p-4 space-y-3">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      <div>
                        <p className="font-medium">{entry.slotLabel}</p>
                        <p className="text-xs text-muted-foreground">
                      ...
                      <span
                        className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-medium ${
                          currentLog ? getStatusBadgeClasses(currentLog.status) : "bg-muted text-muted-foreground border-border/60"
                        }`}
                      >
                        {currentLog ? currentLog.status.toUpperCase() : "PENDING"}
                      </span>
                    </div>
                    {currentLog?.source === "auto-missed" && (
                      <p className="text-xs text-muted-foreground">
                        Auto-marked as skipped because the scheduled time passed without an update.
                      </p>
                    )}

                    <div className="flex flex-wrap gap-2">
                      {(["taken", "late", "skipped"] as const).map((status) => (
                        <Button
                          key={`${key}-${status}`}
                          variant={currentLog?.status === status ? "default" : "outline"}
                          size="sm"
                          disabled={medicationAdherenceSavingKey === key}
                          onClick={() => void handleMedicationStatusUpdate(entry, status)}
                        >
                          {status === "taken" && "Taken"}
                          {status === "late" && "Late"}
          ...
          <Card variant="glass">
            ...
            <CardContent className="space-y-3">
              {medicationAdherenceSummary.totalExpected === 0 && (
                <p className="text-sm text-muted-foreground">
                  Start with a doctor prescription to track scheduled doses.
                </p>
              )}
              {medicationAdherenceSummary.totalExpected > 0 && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-border/60 p-3">
                      ...
                      <p className="text-2xl font-bold text-success">{medicationAdherenceSummary.adherenceRate}%</p>
                    </div>
                    <div className="rounded-lg border border-border/60 p-3">
                      <p className="text-xs text-muted-foreground">Doses Logged</p>
                      <p className="text-2xl font-bold">{medicationAdherenceSummary.logged}/{medicationAdherenceSummary.totalExpected}</p>
                    </div>
                  </div>
                  {medicationAdherenceSummary.byDay.slice(-7).reverse().map((day) => (
                    <div key={day.date} className="rounded-lg border border-border/60 p-3">
