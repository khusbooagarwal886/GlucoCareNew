...
export type MealLogDoc = MealLogInput & {
  id: string;
  uid: string;
  createdAt?: Date;
  updatedAt?: Date;
...
const mealLogsRef = (uid: string) => collection(db, "users", uid, "mealLogs");

function normalizeMealFoods(foods: MealLogFoodInput[]): MealLogFoodInput[] {
  return foods
    .map((food) => ({
      name: food.name.trim(),
      portion: food.portion.trim(),
      calories: typeof food.calories === "number" && Number.isFinite(food.calories) ? Math.round(food.calories) : undefined,
      carbs: typeof food.carbs === "number" && Number.isFinite(food.carbs) ? Math.round(food.carbs) : undefined,
    }))
    .filter((food) => food.name && food.portion);
}

export async function createMealLog(uid: string, input: MealLogInput): Promise<MealLogDoc> {
  const foods = normalizeMealFoods(input.foods);
  if (!foods.length) {
    throw new Error("Add at least one food item.");
  }

  const ref = await addDoc(mealLogsRef(uid), {
    uid,
...
export async function listMealLogs(uid: string): Promise<MealLogDoc[]> {
  const q = query(mealLogsRef(uid), orderBy("loggedAt", "desc"));
  const snap = await getDocs(q);
  return snap.docs.map((d) => {
    const raw = d.data() as Record<string, unknown>;
    return {
      id: d.id,
      uid: String(raw.uid ?? uid),
      mealType: (raw.mealType as MealType) ?? "breakfast",
      loggedAt: parseFirestoreDate(raw.loggedAt) ?? new Date(),
      foods: Array.isArray(raw.foods)
        ? raw.foods.map((food) => ({
            name: String((food as { name?: unknown }).name ?? ""),
            portion: String((food as { portion?: unknown }).portion ?? ""),
            calories:
              typeof (food as { calories?: unknown }).calories === "number"
                ? Number((food as { calories?: unknown }).calories)
                : undefined,
            carbs:
              typeof (food as { carbs?: unknown }).carbs === "number"
                ? Number((food as { carbs?: unknown }).carbs)
                : undefined,
          }))
        : [],
...
export type NotificationDoc = {
  id: string;
  type: "appointment" | "care-plan" | "medication-adherence";
  title: string;
  message: string;
  createdAt: Date;
  isRead: boolean;
  ...
  adherenceSlot?: MedicationReminderSlot;
...
function toNotificationDoc(d: { id: string; data: () => Record<string, unknown> }): NotificationDoc {
  ...
  return {
    ...
    adherenceStatus: (raw.adherenceStatus as MedicationAdherenceStatus | undefined) ?? undefined,
    adherenceSlot: (raw.adherenceSlot as MedicationReminderSlot | undefined) ?? undefined,
    adherenceDate: (raw.adherenceDate as string | undefined) ?? undefined,
  };
}
...
export type MedicationAdherenceStatus = "taken" | "late" | "skipped";
export type MedicationAdherenceSource = "manual" | "auto-missed";
...
export type MedicationAdherenceLogDoc = MedicationScheduleEntry & {
  id: string;
  patientUid: string;
  status: MedicationAdherenceStatus;
  source: MedicationAdherenceSource;
  loggedAt?: Date;
  createdAt?: Date;
  updatedAt?: Date;
};
...
const medicationAdherenceRef = (uid: string) => collection(db, "users", uid, "medicationAdherence");
...
function medicationAdherenceLogId(date: string, slot: MedicationReminderSlot): string {
  return `${date}_${slot}`.replace(/[^a-zA-Z0-9_-]/g, "_");
}

function medicationMissedNotificationId(date: string, slot: MedicationReminderSlot): string {
  return `missed_${date}_${slot}`.replace(/[^a-zA-Z0-9_-]/g, "_");
}

function getReminderSlotDateTime(scheduleDate: string, slot: MedicationReminderSlot): Date | null {
  const timing = MEDICATION_REMINDER_SLOTS.find((item) => item.slot === slot);
  if (!timing) return null;
  const scheduledAt = new Date(`${scheduleDate}T00:00:00`);
  if (Number.isNaN(scheduledAt.getTime())) return null;
  scheduledAt.setHours(timing.hour, timing.minute, 0, 0);
  return scheduledAt;
}

function getMedicationSlotPeriodLabel(slot: MedicationReminderSlot): "morning" | "afternoon" | "night" {
  if (slot.startsWith("afternoon")) return "afternoon";
...
function toMedicationAdherenceLogDoc(
  uid: string,
  ...
  return {
    id: d.id,
    patientUid: String(raw.patientUid ?? uid),
    scheduleDate: String(raw.scheduleDate ?? ""),
    slot: (raw.slot as MedicationReminderSlot) ?? "morning-before-food",
    slotLabel: String(raw.slotLabel ?? ""),
    status: (raw.status as MedicationAdherenceStatus) ?? "taken",
    source: (raw.source as MedicationAdherenceSource) ?? "manual",
    medications: Array.isArray(raw.medications)
      ? raw.medications.map((m) => ({
          name: String((m as { name?: unknown }).name ?? ""),
          dosage: String((m as { dosage?: unknown }).dosage ?? ""),
      ...
      : [],
    loggedAt: parseFirestoreMaybeDate(raw.loggedAt),
    createdAt: parseFirestoreMaybeDate(raw.createdAt),
    updatedAt: parseFirestoreMaybeDate(raw.updatedAt),
  };
}
...
export async function listMedicationScheduleForDate(
  patientUid: string,
  scheduleDate: string
): Promise<MedicationScheduleEntry[]> {
  if (!scheduleDate) return [];

  const { medicines } = await fetchPatientReminderMedicines(patientUid);
  ...
  return MEDICATION_REMINDER_SLOTS.flatMap((slotTiming) => {
    const slotMeds = medicationsBySlot.get(slotTiming.slot);
    if (!slotMeds || slotMeds.length === 0) return [];
    return [
      {
        scheduleDate,
        slot: slotTiming.slot,
        slotLabel: slotTiming.label,
...
export async function listMedicationAdherenceLogs(
  patientUid: string,
  daysBack = 14
): Promise<MedicationAdherenceLogDoc[]> {
  const q = query(
    medicationAdherenceRef(patientUid),
    orderBy("scheduleDate", "desc"),
    limit(Math.max(daysBack * 6, 12))
  );
  const snap = await getDocs(q);
  return snap.docs.map((d) => toMedicationAdherenceLogDoc(patientUid, d));
}

export async function autoMarkMissedMedicationDoses(
  patientUid: string,
  daysBack = 7
): Promise<number> {
  const recentDates = Array.from({ length: daysBack }, (_, index) => {
    const d = new Date();
    d.setDate(d.getDate() - (daysBack - 1 - index));
    return formatDateYmd(d);
  });

  const [schedulesByDate, logs] = await Promise.all([
    Promise.all(recentDates.map((date) => listMedicationScheduleForDate(patientUid, date))),
    listMedicationAdherenceLogs(patientUid, daysBack),
  ]);

  const scheduleEntries = schedulesByDate.flat();
  if (scheduleEntries.length === 0) return 0;

  const existingByKey = new Map(logs.map((log) => [`${log.scheduleDate}_${log.slot}`, log] as const));
  const now = Date.now();
  ...
  for (const entry of scheduleEntries) {
    ...
    const scheduledAt = getReminderSlotDateTime(entry.scheduleDate, entry.slot);
    if (!scheduledAt || scheduledAt.getTime() > now) continue;

    const ref = doc(medicationAdherenceRef(patientUid), medicationAdherenceLogId(entry.scheduleDate, entry.slot));
    batch.set(ref, {
      patientUid,
      scheduleDate: entry.scheduleDate,
      slot: entry.slot,
      slotLabel: entry.slotLabel,
      status: "skipped" as MedicationAdherenceStatus,
      source: "auto-missed" as MedicationAdherenceSource,
      medications: entry.medications,
      loggedAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
      createdAt: serverTimestamp(),
    });
    const periodLabel = getMedicationSlotPeriodLabel(entry.slot);
    batch.set(doc(notificationsRef(patientUid), medicationMissedNotificationId(entry.scheduleDate, entry.slot)), {
      type: "medication-adherence",
      title: `You missed your ${periodLabel} dose`,
      message: `Your ${entry.slotLabel.toLowerCase()} medication dose was auto-marked as skipped because it was not updated after the scheduled time.`,
      adherenceStatus: "skipped" as MedicationAdherenceStatus,
      adherenceSlot: entry.slot,
      adherenceDate: entry.scheduleDate,
      isRead: false,
      createdAt: serverTimestamp(),
    });
    created += 1;
...
export async function upsertMedicationAdherenceLog(
  patientUid: string,
  input: {
    scheduleDate: string;
    slot: MedicationReminderSlot;
  ...
  const docId = medicationAdherenceLogId(input.scheduleDate, input.slot);
  const ref = doc(medicationAdherenceRef(patientUid), docId);
  const existing = await getDoc(ref);

  let slotLabel = input.slotLabel;
  let medications = input.medications;

  if (!slotLabel || !medications || medications.length === 0) {
    const schedule = await listMedicationScheduleForDate(patientUid, input.scheduleDate);
    const matched = schedule.find((item) => item.slot === input.slot);
    slotLabel = slotLabel ?? matched?.slotLabel ?? input.slot;
    medications = medications ?? matched?.medications ?? [];
  }
...
export async function uploadMealPhoto(
  ...
  if (!file.type.startsWith("image/")) {
    throw new Error("Please upload a valid meal photo.");
  }
  if (file.size > MAX_MEAL_PHOTO_BYTES) {
    throw new Error("Meal photo should be under 5 MB.");
  }

  const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
  const photoPath = `mealPhotos/${patientUid}/${Date.now()}-${safeName}`;
